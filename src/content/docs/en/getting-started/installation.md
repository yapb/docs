...
:::caution
The installer program automatically detects whether you use metamod, and will patch the `plugins.ini` file automatically for you, adding an entry to point to `yapb.dll`. If there is no metamod, the installer will patch the `liblist.gam` file to point to `yapb.dll`.
...
:::note
There is an unofficial binary called `hlds_osx` provided by AlliedModders LLC allowing you to run HLDS under macOS. You can try downloading it [here](https://forums.alliedmods.net/showpost.php?p=2041538&postcount=11).
